  ...
  for x in [1..size] do
    for y in [1..size] do
      ...
      for z in [1..size] do
        xz := lst[x][z];
        yz := lst[y][z];
        if lst[xy][xz] <> lst[yx][yz] then
          return fail;
        fi;
      od;
    od;
  od;
  return Objectify(CycleSetType, rec(size := size, matrix := lst));
end);
...
InstallMethod(ViewObj,
  ...
  [ IsCycleSet ],
  function(obj)
  Print("A cycle set of size ", obj!.size);
end);
...
function(obj)
  ...
  for x in [1..obj!.size] do
    ...
    od;
  od;
  rperms := List(Permutations(obj), x->ListPerm(Inverse(x), obj!.size));
  return YB(lperms, rperms);
  #return YB(List(Permutations(obj), x->ListPerm(Inverse(x), obj!.size)), obj!.matrix);
end);
